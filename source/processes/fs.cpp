...
public:
    ...
    static std::string BuildBasePath(FSContext& context, const ProgramInfo& program_info) {
        ...
        if (program_info.media_type == 1 /* SD */) {
            return (HostSdmcDirectory(context.settings) /
                    fmt::format("Nintendo 3DS/{:016x}/{:016x}/title/{:08x}/{:08x}/data",
                                GetId0(context), GetId1(context), program_id_high, program_id_low))
                .string();
